...
public class SRSInfo {
    ...
    private Boolean isAxisXY=null;
    private Boolean isGeographic=null;
    private Boolean isSRSRecognised=false;
    private Boolean isWktDefault=false;
    private Envelope domainEnvelope=null;
    private Double domainRangeX=null;
    ...
    public static SRSInfo resolveSRS(String srsURI,OntModel model) {
    	SRSInfo result=new SRSInfo();
    	String wkt=RDFCRSToWKT.getCRSFromTripleStore(srsURI, model, "wkt");
    	CoordinateReferenceSystem crs;
		try {
			crs = CRS.fromWKT(wkt);
	    	result.crs=crs;
	        result.isAxisXY = checkAxisXY(crs);
	        result.unitsOfMeasure = new UnitsOfMeasure(crs);
	        result.isSRSRecognised = true;
	        result.isGeographic = crs instanceof GeographicCRS;
	        result.isWktDefault = srsURI.equals(SRS_URI.DEFAULT_WKT_CRS84);
	        result.domainEnvelope = buildDomainEnvelope(crs, result.isAxisXY);
	        result.domainRangeX = Math.abs(result.domainEnvelope.getMinX()) + Math.abs(result.domainEnvelope.getMaxX());
	    	return result;
    ...
    public static SRSInfo resolveSRS(String srsURI,String endpointURI) {
    	SRSInfo result=new SRSInfo();
    	String wkt=RDFCRSToWKT.getCRSFromTripleStore(srsURI, endpointURI, "wkt");
    	CoordinateReferenceSystem crs;
		try {
			crs = CRS.fromWKT(wkt);
	    	result.crs=crs;
